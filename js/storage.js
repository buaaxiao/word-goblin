...
function setDataSourceHint(source) {
  ...
  if (source === "本地") {
    el.textContent =
      "当前词库来自本地缓存。如需获取共享词库最新版本，可点「☁️ 从共享词库同步」。";
  } else if (source === "共享") {
    el.textContent =
      "当前词库来自共享 data.json，已缓存到本地。之后修改都保存在本地。";
  } else {
...
function normalizeData() {
  ...
  for (const ch of data.chapters) {
    ...
    for (const w of ch.words) {
      ...
      w.lastErrorDate = w.lastErrorDate || "";
      w.lastCorrectDate = w.lastCorrectDate || "";
      w.scoreCount = w.scoreCount || 0;
      w.scoreSum = w.scoreSum || 0;
    }
  }
}
function saveData() {
...
function jumpToWrongWords() {
  for (let i = 0; i < data.chapters.length; i++) {
    const hasWrong = data.chapters[i].words.some(
      (w) => (w.wrongCount || 0) > 0,
    );
    if (hasWrong) {
      currentChapter = i;
